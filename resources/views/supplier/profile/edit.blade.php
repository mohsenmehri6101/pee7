...
@section('page_header')
    <li class="breadcrumb-item"><a href="{{ route('supplier.profile.edit') }}">ویرایش اطلاعات</a></li>
    <li class="breadcrumb-item"><a class="text-secondary" href="{{ route('supplier.profile.index') }}">پروفایل</a></li>
@endsection
...
<div class="row">
    ...
    <div class="col-md-6">
        <form action="{{ route('supplier.profile.update' , ['id' => $user->id]) }}" method="post" enctype="multipart/form-data" >
            {{ csrf_field() }}
            <div class="card card-primary card-outline">
                <div class="card-header">
                    <h3 class="card-title">ویرایش اطلاعات<a href="{{ route('supplier.profile.index') }}"> {{ $user->name }}</a></h3>
                </div>
                <!-- /.card-header -->
                <div class="card-body">
                    ...
                    <div class="form-group">
                        <select name="city" id="city_id"  class="form-control select2" data-live-search="true">
                            <option value="{{ $user->location->city }}"  selected >{{ $user->location->city }}</option>
                        </select>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        <textarea name="about" id="" class="form-control textarea" rows="5" placeholder="توضیحات بیشتر(اختیاری...)">{{ $user->supplier->about }}</textarea>
                    </div>
                    <div class="form-group">
                        @if($user->image)
