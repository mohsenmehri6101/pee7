...
@section('page_header')
    <li class="breadcrumb-item"><a href="{{ route('supplier.profile.create') }}">اطلاعات تکمیلی</a></li>
...
@section('content')
    ...
    <div class="col-md-9">
        <form action="{{ route('supplier.profile.store' , ['id' => $user->id]) }}" method="post" enctype="multipart/form-data" >
            {{ csrf_field() }}
            <div class="card card-primary card-outline">
                <div class="card-header">
                    <h3 class="card-title">اطلاعات تکمیلی <a href="{{ route('supplier.profile.index') }}"> {{ $user->name }}</a></h3>
                </div>
                <!-- /.card-header -->
                <div class="card-body">
