...
@section('page_header')
    <li class="breadcrumb-item"><a class="text-info" href="{{ route('user.profile.create') }}">تکمیل اطلاعات</a></li>
    <li class="breadcrumb-item"><a class="text-secondary" href="{{ route('user.profile.index') }}">پروفایل</a></li>
@endsection

@section('content')
    <div class="row">
    @include('person.layouts.errors')
    <div class="col-md-9">
        <form action="{{ route('user.profile.store' , ['id' => $user->id]) }}" method="post" enctype="multipart/form-data" >
            {{ csrf_field() }}
            <div class="card card-primary card-outline">
                <div class="card-header">
                    <h3 class="card-title">اطلاعات تکمیلی <a href="{{ route('user.profile.index') }}"> {{ $user->name }}</a></h3>
                </div>
                <!-- /.card-header -->
                <div class="card-body">
                    ...
                    <div class="form-group">
                        <input class="form-control" name="self_id" value="{{old('self_id')}}" placeholder="شماره ملی">
                    </div>
                    <div class="form-group">
